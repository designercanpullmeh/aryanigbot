...
async def gc_rename_loop(username, cl, gid, get_titles, delay):
    ...
    while True:
        ...
        j += 1
        await asyncio.sleep(delay)
async def send_round(username, cl, groups):
    total = len(groups)
    # pick message block for this round
    active_block = None
    if MESSAGE_BLOCKS:
        # simple rotation per round is optional; can just use 0
        active_block = MESSAGE_BLOCKS[0]
    ...
    for index, thread in enumerate(groups, start=1):
        ...
        await asyncio.sleep(MSG_DELAY)   # 40s between GCs


async def rename_round(username, cl, groups):
    total = len(groups)
    ...
    for index, thread in enumerate(groups, start=1):
        ...
        if current_title != title:
            try:
                success = await asyncio.to_thread(rename_thread, cl, gid, title)
                if success:
                    ui_log(username, f"💠 → {title} (GC {index}/{total})")
                else:
                    ui_log(username, f"⚠ Rename failed GC {index}")
            except Exception:
                ui_log(username, f"⚠ Rename error GC {index}")
        else:
            ui_log(username, f"💠 OK → {title} (GC {index}/{total})")

        await asyncio.sleep(240)   # 240s between GCs

async def message_loop(username, cl):
    """Send messages GC1→GC2→... with 40s between GCs, forever."""
    while True:
        try:
        ...
        active_block = None
        if MESSAGE_BLOCKS:
            # simple rotation: use (time-based) or just first block
            active_block = MESSAGE_BLOCKS[0]
        ...
        for index, thread in enumerate(groups, start=1):
            ...
            try:
                ...
                pass
            ui_log(username, f"📨 → GC {index}/{total}")
            await asyncio.sleep(MSG_DELAY)   # 40s between each GC

async def namechange_loop(username, cl):
    """Rename GCs GC1→GC2→... with 240s between GCs, forever."""
    last_title = None
    used_titles = set()
    ...
    while True:
        ...
        ui_log(username, f"⏳ NC ROUND | GCS → {total} | Titles → {len(titles)}")

        for index, thread in enumerate(groups, start=1):
            gid = thread.id

            available = [t for t in titles if t != last_title and t not in used_titles]
            if not available:
                used_titles.clear()
                available = [t for t in titles if t != last_title]
...
async def worker(username, password, proxy, cl):
    """Run message and namechange loops in parallel, forever."""
    msg_task = asyncio.create_task(message_loop(username, cl))
    nc_task = asyncio.create_task(namechange_loop(username, cl))
