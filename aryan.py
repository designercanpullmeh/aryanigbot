...
async def worker(username, password, proxy, cl):
    ...
    while True:
        ...
        total = len(groups)
        if total == 0:
            ui_log(username, "⚠ No GCs found")
            await asyncio.sleep(60)
            continue

        ui_log(username, f"⏳ ROUND {round_number} | GCS → {total}")

        # run send + rename rounds in parallel for this account
        send_task = asyncio.create_task(send_round(username, cl, groups))
        rename_task = asyncio.create_task(rename_round(username, cl, groups))

        await asyncio.gather(send_task, rename_task)
